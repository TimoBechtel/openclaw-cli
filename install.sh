...
REPO="TimoBechtel/openclaw-cli"
BIN_NAME="openclaw-cli"
...
detect_asset() {
  ...
  case "$os" in
    Darwin)
      case "$arch" in
        arm64) printf "openclaw-cli-darwin-arm64" ;;
        x86_64|amd64) printf "openclaw-cli-darwin-x64" ;;
        *) fail "Unsupported macOS architecture: $arch" ;;
      esac
      ;;
    Linux)
      case "$arch" in
        x86_64|amd64) printf "openclaw-cli-linux-x64" ;;
        *) fail "Unsupported Linux architecture: $arch" ;;
      esac
      ;;
    MINGW*|MSYS*|CYGWIN*)
      case "$arch" in
        x86_64|amd64) printf "openclaw-cli-windows-x64.exe" ;;
...
resolve_tag() {
  if [ -n "${OPENCLAW_CLI_VERSION:-}" ]; then
    if [[ "$OPENCLAW_CLI_VERSION" == v* ]]; then
      printf "%s" "$OPENCLAW_CLI_VERSION"
    else
      printf "v%s" "$OPENCLAW_CLI_VERSION"
    fi
    return
  fi
